...
import { ActivityIndicator, FlatList, StyleSheet, Text, TextInput, TouchableOpacity, View, VirtualizedList } from 'react-native';
...
export const UsuarioEjerciciosScreen = () => {
  ...
  const fetchData = async (reset: boolean = true) => {
    ...
    try {
      ...
      const response = await routineApi.get('/ejercicios/filtrarCreador', {
        ...config,
        params: {
          username_creador: user?.email === "admin@admin.com" || user?.username === "admin@admin.com" ? "admin" : (user?.email || user?.username),
      ...
      setEjercicios((prevEjercicios) => ({
        contenido: reset ? response.data.contenido : [...prevEjercicios.contenido, ...response.data.contenido],
